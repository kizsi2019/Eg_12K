...
namespace CRUT
{
    public partial class Form1 : Form
    {
        ...
        private void btn7_Click(object sender, EventArgs e)
        {
            ...
            MySqlConnection con = new MySqlConnection(conString);
            con.Open();
            string sql = "SELECT DISTINCT keszito.nev FROM keszito, kapcsolat WHERE keszito.id=kapcsolat.keszitoid AND kapcsolat.filmid IN (SELECT kapcsolat.filmid FROM kapcsolat, keszito WHERE kapcsolat.keszitoid=keszito.id AND keszito.nev=\"Clint Eastwood\")\r\n\tAND nev<>\"Clint Eastwood\";";
            MySqlCommand cmd = new MySqlCommand(sql, con);
            MySqlDataReader reader = cmd.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(reader);
